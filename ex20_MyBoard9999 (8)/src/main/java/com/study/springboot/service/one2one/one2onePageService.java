...
@Component
public class one2onePageService {
	@Autowired
	...
	public List<one2oneDto> PagingList(
		...
		int totalList = iOne2oneDao.one2oneCount(); //전체 게시글개수
		...
		List<one2oneDto> list = iOne2oneDao.betweenList(startList,endList); // 게시물 리스트 범위 출력
